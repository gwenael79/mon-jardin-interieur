...
serve(async (req) => {
  ...
  try {
    const { prenom, nom, email, telephone, montant, niveau, label, citation, fleur_image, upgrade } = await req.json()
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { prenom, nom, email, telephone: telephone ?? '', niveau, citation: citation ?? '', fleur_image: fleur_image ?? '', upgrade: upgrade ? 'true' : 'false', source: 'cercle_fondateurs' },
