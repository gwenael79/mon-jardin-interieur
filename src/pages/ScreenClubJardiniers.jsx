  ...
  return (
    <div style={{
      ...
      <div
        ...
        onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)' }}
        title={isSent ? `Disponible dans ${cooldown ?? '…'}` : `Offrir un élan 💐 sur ${weakest.name}`}
      >
...
function ScreenClubJardiniers({ userId, awardLumens, onCoeurSeen }) {
  ...
  return (
    <>
      ...
      {showJardin   && <ModalJardin userId={userId} myName={myName} bouquetIds={bouquetIds} onClose={() => { setShowJardin(false); loadMessages() }} onCoeurSent={({ receiverName, zone }) => { showToast(`💐 Élan offert à ${receiverName} !`); awardLumens?.(1, 'coeur_envoye', { zone }); loadMessages() }} />}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '20px 14px' : '36px 40px' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, padding: '18px 20px', borderRadius: 16, background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)' }}>
            <p style={{ margin: '0 0 16px', fontSize: 12, color: 'rgba(238,232,218,.55)', lineHeight: 1.75, textAlign: 'center', fontStyle: 'italic' }}>
              ...
              {[
                ...
                <div key={item.title} style={{ display: 'flex', gap: 12, paddingTop: i > 0 ? 14 : 0, borderTop: i > 0 ? '1px solid rgba(255,255,255,.05)' : 'none' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'rgba(238,232,218,.42)', lineHeight: 1.7 }}>{item.desc}</div>
                  </div>
                </div>
              ))}
            </div>
