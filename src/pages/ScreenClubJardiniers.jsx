  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: `conic-gradient(${isFragile ? 'rgba(255,140,60,.85)' : 'var(--green)'} ${vit * 3.6}deg, rgba(255,255,255,.07) 0deg)`,
      ...
      <div style={{
        ...
        color: isFragile ? 'var(--gold-warm)' : 'var(--gold)', textAlign: 'center',
      ...
      <div
        ...
        style={{
          ...
          fontSize: 16, opacity: !isPremium ? 0.4 : sending ? .5 : 1,
      ...
      {expanded && badge > 0 && (
        <div style={{ width: '100%', borderTop: '1px solid rgba(255,200,100,.12)', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {(pendingMercisForFleur ?? []).map(c => {
            const z = ZONE_MAP[c.zone]
            return (
              <div key={c.id} style={{ fontSize: 10, color: 'var(--text3)', fontStyle: 'italic', lineHeight: 1.5 }}>
                "{c.message_ia}"
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          ...
          color: isFragile ? 'var(--gold-warm)' : 'var(--gold)',
      ...
      <div
        ...
        style={{
          ...
          background: isSent ? 'rgba(255,255,255,.04)' : 'rgba(150,212,133,.15)',
          border: `1px solid ${isSent ? 'rgba(255,255,255,.07)' : 'rgba(150,212,133,.4)'}`,
          boxShadow: isSent ? 'none' : '0 0 8px rgba(150,212,133,.25)',
      ...
      >
        {!isPremium
          ...
          : <>
              <div style={{ fontSize: isSent ? 13 : 15, filter: isSent ? 'none' : 'drop-shadow(0 0 4px rgba(150,212,133,.5))', opacity: isSent ? .25 : 1, lineHeight: 1 }}>
                {sending ? '…' : '💐'}
              </div>
      ...
      {expanded && badge > 0 && (
        <div style={{ width: '100%', borderTop: '1px solid rgba(255,200,100,.1)', paddingTop: 8, marginTop: 4, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {(pendingMercisForFleur ?? []).map(c => {
            ...
            return (
              <div key={c.id}>
                <div style={{ fontSize: 10, color: 'var(--text3)', fontStyle: 'italic', lineHeight: 1.5 }}>"{c.message_ia}"</div>
                <div onClick={() => isPremium ? onMerci?.(c.id, c.sender_id) : onUpgrade?.()} style={{ marginTop: 4, display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 8px', borderRadius: 100, fontSize: 9, background: 'rgba(255,200,100,.1)', border: '1px solid rgba(255,200,100,.2)', color: 'rgba(255,220,140,.9)', cursor: isPremium ? 'pointer' : 'not-allowed', opacity: isPremium ? 1 : 0.4 }}>
    ...
    <Modal onClose={onClose} maxWidth={500}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '8px 24px 16px' }}>
        ...
        <div onClick={handleJoin} style={{
          ...
          background: !isPremium ? 'rgba(255,255,255,0.04)' : joined ? 'rgba(232,196,100,.15)' : 'rgba(150,212,133,.10)',
          border: `1px solid ${!isPremium ? 'rgba(255,255,255,.10)' : joined ? 'rgba(232,196,100,.50)' : 'rgba(150,212,133,.35)'}`,
    ...
    <Modal onClose={onClose} maxWidth={440}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '20px 24px 12px', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 300, color: 'var(--gold)', marginBottom: 6 }}>Offrir un bouquet</div>
          <div style={{ fontSize: 11, color: 'var(--text3)', lineHeight: 1.65, maxWidth: 320 }}>
      ...
      <div style={{ padding: '12px 20px 4px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 16, fontWeight: 300, color: 'rgba(232,212,168,.7)' }}>Ces jardins ont besoin d'un peu d'énergie</div>
        <div style={{ fontSize: 9, color: 'var(--text3)', flexShrink: 0, marginLeft: 8 }}>{list.length}/20</div>
      </div>
      <div style={{ padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: 7 }}>
        {!ready
          ...
          : list.length === 0
            ? <div style={{ textAlign: 'center', padding: 32, color: 'var(--text3)', fontSize: 12 }}>Toutes les fleurs sont en bonne santé 🌿</div>
            : list.map(fleur => (
                <BouquetCard
                  key={fleur.id}
                  fleur={fleur}
...
function ModalBouquet({ userId, myName, onClose, isPremium = false, onUpgrade }) {
  ...
  return (
    <Modal onClose={onClose} maxWidth={400}>
      {/* Intro coeur */}
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '20px 24px 12px', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
        <div style={{ fontSize: 64, lineHeight: 1, filter: 'drop-shadow(0 0 18px rgba(210,180,60,.5))' }}>🌻</div>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 300, color: 'var(--gold)', marginBottom: 6 }}>Mes ami(e)s jardinier(e)s</div>
          <div style={{ fontSize: 11, color: 'var(--text3)', lineHeight: 1.65, maxWidth: 300 }}>
      ...
      <div style={{ padding: '12px 20px 4px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        ...
        <div style={{ fontSize: 9, color: 'var(--text3)', flexShrink: 0, marginLeft: 8 }}>{bouquet.length} jardinier{bouquet.length !== 1 ? 's' : ''}</div>
      </div>
      <div style={{ padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: 7 }}>
        {loading
          ...
          : bouquet.length === 0
            ? (
              <div style={{ background: 'rgba(150,212,133,.03)', border: '1px solid rgba(150,212,133,.1)', borderRadius: 14, padding: '28px 20px', textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text3)', lineHeight: 1.65 }}>Envoyez des 💐 dans le jardin — quand un(e) ami(e) vous remercie, il/elle rejoint vos ami(e)s jardinier(e)s.</div>
              </div>
            )
            : bouquet.map(fleur => (
                <BouquetCard
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '20px 14px' : '36px 40px' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 7 }}>{todayStr}</div>
          </div>
          ...
          {/* Texte de bienvenue */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, padding: '18px 20px', borderRadius: 16, background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)' }}>
            <p style={{ margin: '0 0 16px', fontSize: 12, color: 'var(--text3)', lineHeight: 1.75, textAlign: 'center', fontStyle: 'italic' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                <div key={item.title} style={{ display: 'flex', gap: 12, paddingTop: i > 0 ? 14 : 0, borderTop: i > 0 ? '1px solid rgba(255,255,255,.05)' : 'none' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 14, color: item.color, marginBottom: 4, fontWeight: 400 }}>{item.title}</div>
              ...
              {messages.map((c, i) => {
                ...
                return (
                  <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: 'rgba(255,100,100,.04)', border: '1px solid rgba(255,100,100,.1)', borderRadius: 14, animation: `fadeInUp .3s ease ${i * 0.05}s both` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 2 }}>{sender}</div>
                      <div style={{ fontSize: 11, color: 'var(--text3)', fontStyle: 'italic', lineHeight: 1.55 }}>"{c.message_ia}"</div>
                      <div style={{ fontSize: 9, color: z?.color ?? 'var(--green)', marginTop: 3 }}>{z?.icon} {z?.name} · {timeAgo(c.created_at)}</div>
                    </div>
                    <div onClick={() => isPremium ? handleMerci(c.id, c.sender_id) : onUpgrade?.()} style={{ flexShrink: 0, minHeight: 32, padding: '0 14px', borderRadius: 100, fontSize: 10, background: 'rgba(255,200,100,.1)', border: '1px solid rgba(255,200,100,.25)', color: isPremium ? 'rgba(255,220,140,.9)' : 'rgba(255,220,140,.35)', cursor: isPremium ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', WebkitTapHighlightColor: 'transparent' }}>{isPremium ? '🙏 Merci' : '🔒 Merci'}</div>
                  </div>
                )
              })}
            </div>
