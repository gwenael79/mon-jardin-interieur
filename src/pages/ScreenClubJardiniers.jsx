...
function BouquetCard({ fleur, userId, senderName, alreadySent, onCoeurSent, badge, onBadgeClick, expanded, pendingMercisForFleur, onMerci, intentionMode, isPremium = false, onUpgrade }) {
  ...
  return (
    <div style={{
      background: isFragile ? 'rgba(var(--gold-rgb),.05)' : 'var(--surface-1)',
      border: `1px solid ${isFragile ? 'rgba(var(--gold-rgb),.16)' : 'var(--surface-2)'}`,
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: isFragile ? 'rgba(var(--gold-rgb),.9)' : 'rgba(var(--badge-lvl1-rgb),.7)',
        }}>{vit}%</div>
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          fontFamily: "'Cormorant Garamond',serif", fontSize:'var(--fs-h4, 13px)', color: isFragile ? 'var(--gold-warm)' : 'var(--gold)',
    ...
    <Modal onClose={onClose} maxWidth={440}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '20px 24px 12px', borderBottom: '1px solid var(--surface-2)' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize:'var(--fs-h3, 18px)', fontWeight: 300, color: 'var(--gold)', marginBottom: 6 }}>Offrir un bouquet</div>
          <div style={{ fontSize:'var(--fs-h5, 11px)', color: 'var(--text3)', lineHeight: 1.65, maxWidth: 320 }}>
      ...
      <div style={{ padding: '12px 20px 4px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize:'var(--fs-h3, 16px)', fontWeight: 300, color: 'rgba(var(--gold-warm-rgb),.7)' }}>Ces jardins ont besoin d'un peu d'énergie</div>
        <div style={{ fontSize:'var(--fs-h5, 9px)', color: 'var(--text3)', flexShrink: 0, marginLeft: 8 }}>{list.length}/20</div>
      </div>
      <div style={{ padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: 7 }}>
        {!ready
          ...
          : list.length === 0
            ? <div style={{ textAlign: 'center', padding: 32, color: 'var(--text3)', fontSize:'var(--fs-h5, 12px)' }}>Toutes les fleurs sont en bonne santé 🌿</div>
            : list.map(fleur => (
                <BouquetCard
                  key={fleur.id}
                  fleur={fleur}
  ...
  return (
    <Modal onClose={onClose} maxWidth={400}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '20px 24px 12px', borderBottom: '1px solid var(--surface-2)' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize:'var(--fs-h3, 18px)', fontWeight: 300, color: 'var(--gold)', marginBottom: 6 }}>Mes ami(e)s jardinier(e)s</div>
          <div style={{ fontSize:'var(--fs-h5, 11px)', color: 'var(--text3)', lineHeight: 1.65, maxWidth: 300 }}>
            Offrez une présence bienveillante à vos ami(e)s jardiniers — un lien de confiance, cultivé ensemble.
          </div>
        </div>
      </div>
      <div style={{ padding: '12px 20px 4px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize:'var(--fs-h3, 16px)', fontWeight: 300, color: 'rgba(var(--gold-warm-rgb),.7)' }}>Mes ami(e)s jardinier(e)s</div>
        <div style={{ fontSize:'var(--fs-h5, 9px)', color: 'var(--text3)', flexShrink: 0, marginLeft: 8 }}>{bouquet.length} jardinier{bouquet.length !== 1 ? 's' : ''}</div>
      </div>
      <div style={{ padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: 7 }}>
        {loading
          ...
          : bouquet.length === 0
            ? (
              <div style={{ background: 'rgba(var(--green-rgb),.03)', border: '1px solid rgba(var(--green-rgb),.1)', borderRadius: 14, padding: '28px 20px', textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
                ...
                <div style={{ fontSize:'var(--fs-h5, 11px)', color: 'var(--text3)', lineHeight: 1.65 }}>Envoyez des 💐 dans le jardin — quand un(e) ami(e) vous remercie, il/elle rejoint vos ami(e)s jardinier(e)s.</div>
              </div>
            )
            : bouquet.map(fleur => (
                <BouquetCard
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          <div>
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: isMobile ? 34 : 50, fontWeight: 300, color: 'var(--gold)', lineHeight: 1.0, letterSpacing: '.02em' }}>
              Club des<br /><em style={{ color: 'rgba(var(--green-rgb),.85)' }}>Jardiniers</em>
            </div>
            <div style={{ fontSize:'var(--fs-h5, 10px)', color: 'var(--text3)', marginTop: 7 }}>{todayStr}</div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, padding: '18px 20px', borderRadius: 16, background: 'var(--surface-1)', border: '1px solid var(--surface-2)' }}>
            <p style={{ margin: '0 0 16px', fontSize:'var(--fs-h5, 12px)', color: 'var(--text3)', lineHeight: 1.75, textAlign: 'center', fontStyle: 'italic' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                <div key={item.title} style={{ display: 'flex', gap: 12, paddingTop: i > 0 ? 14 : 0, borderTop: i > 0 ? '1px solid var(--surface-2)' : 'none' }}>
                  ...
                  <div>
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize:'var(--fs-h4, 14px)', color: item.color, marginBottom: 4, fontWeight: 400 }}>{item.title}</div>
                    <div style={{ fontSize:'var(--fs-h5, 11px)', color: 'var(--text3)', lineHeight: 1.7 }}>{item.desc}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {messages.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize:'var(--fs-h5, 9px)', color: 'var(--text3)', letterSpacing: '.12em', textTransform: 'uppercase' }}>✦ Des petites attentions à votre égard !</div>
              {messages.map((c, i) => {
                const z = ZONE_MAP[c.zone]
                const sender = flowerName(c.sender)
                return (
                  <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: 'rgba(var(--red-rgb),.04)', border: '1px solid rgba(var(--red-rgb),.1)', borderRadius: 14, animation: `fadeInUp .3s ease ${i * 0.05}s both` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize:'var(--fs-h5, 11px)', color: 'var(--text3)', marginBottom: 2 }}>{sender}</div>
                      <div style={{ fontSize:'var(--fs-h5, 11px)', color: 'var(--text3)', fontStyle: 'italic', lineHeight: 1.55 }}>"{c.message_ia}"</div>
