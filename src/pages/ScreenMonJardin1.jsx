...
function PlantSVG({ health = 5, gardenSettings = DEFAULT_GARDEN_SETTINGS, lumensLevel = 'faible', lumensTotal = 0, compact = false, bare = false }) {
  ...
  */
  ...
  /* ── Scintillements — étoiles lumineuses sur une zone ── */
  const sparkleAt = (px, py, count, col, spreadX, spreadY, baseDelay=0) => {
    const pts = Array.from({length: count}, (_, i) => {
      // positions déterministes mais variées autour du centre
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid slice" fill="none" style={{display:'block'}}>
      ...
      <g clipPath={`url(#${id}cl)`}>
        ...
        {/* ── SCINTILLEMENTS — zones qui évoluent selon la progression ── */}
        {sparkleOpacity > 0 && <g pointerEvents="none" style={{ opacity: sparkleOpacity, transition: "opacity 0.1s linear" }}>

          {/* Racines : dès 10% */}
          {r > 0.10 && r < 0.55 && sparkleAt(
            cx + 18, gY + 6,
            3, 'rgba(255,222,60,0.90)',
            18, 8, 0
          )}

          {/* Tige : scintille le long de la tige */}
          {r > 0.12 && sparkleAt(
            cx + 4, gY - sH * 0.45,
            r < 0.45 ? 3 : 2,
            'rgba(255,222,60,0.90)',
            10, sH * 0.08, 0.3
          )}
          {r > 0.30 && sparkleAt(
            cx - 5, gY - sH * 0.72,
          ...
          {/* Feuilles : dès 25% */}
          {r > 0.25 && sparkleAt(
            cx - 20 - 12 * r, gY - sH * 0.55,
            3, 'rgba(255,222,60,0.90)',
            12, 7, 0.5
          )}
          {r > 0.35 && sparkleAt(
            cx + 22 + 10 * r, gY - sH * 0.38,
            3, 'rgba(255,222,60,0.90)',
            12, 7, 1.1
          )}

          {/* Bourgeon (45–65%) */}
          {r >= 0.45 && r < 0.65 && sparkleAt(
            cx, flwY,
            5, 'rgba(255,222,60,0.90)',
            fS * 1.6, fS * 1.2, 0
          )}
          ...
          {r >= 0.65 && sparkleAt(
            cx, flwY,
            6, 'rgba(255,222,60,0.90)',
            fS * 2.2, fS * 1.8, 0
          )}
          {r >= 0.65 && sparkleAt(
            cx, flwY,
            4, 'rgba(255,222,60,0.90)',
            fS * 1.1, fS * 0.9, 1.0
          )}
...
function BoiteAGraines({ userId, inline, aiMessage = null, aiLoading = false }) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('user_id', userId)
      .then(({ count }) => { if (count) setGrainesCount(count) })
  }, [userId])
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {inline && (grainesCount > 0 || savedToday) && (
          <div style={{ display:'flex', justifyContent:'center' }}>
            <div onClick={openModal} style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 26px', borderRadius:100, background:'rgba(60,120,60,0.12)', border:'1px solid rgba(60,120,60,0.3)', cursor:'pointer', fontSize: isMobile ? 17 : 18, color:'#1a4a18', fontFamily:"'Jost',sans-serif", fontWeight:500 }}>
              ...
              <span>Voir mes graines ({grainesCount > 0 ? grainesCount : '…'})</span>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div style={{ display:'flex', gap: 10 }}>
              {GRAINE_TAGS.map(t => {
                ...
                const selBg    = isCoeur ? '#fce4ec' : '#e8f5e9'
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      color: selected ? selColor : '#3a3a3a',
                      boxShadow: selected ? `0 2px 8px ${selBorder}44` : 'none',
                    }}
                  >
                    <span style={{ fontSize: 20 }}>{t.emoji}</span>
                    <span style={{ fontSize: 16, fontFamily:"'Jost',sans-serif", fontWeight: selected ? 600 : 400 }}>{t.label}</span>
                  </div>
                )
              })}
            </div>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
                <div style={{ flex:1, display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-start' }}>
                    ...
                    <span style={{ fontFamily:"'Jost',sans-serif", fontSize: isMobile ? 38 : 46, fontWeight:300, color:'rgba(255,255,255,1.0)', lineHeight:1, transition:'color .2s' }}>{animHealth}%</span>
                  </div>
                </div>
