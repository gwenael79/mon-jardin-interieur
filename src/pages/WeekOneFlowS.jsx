...
function DayAudioPlayer({ audioId, zone, dayColor, onValidate, onBack }) {
  ...
  function startGuide() {
    ...
    audio.play().then(() => setState('playing')).catch(() => setState('ended'))
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 10400, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: bg, fontFamily: "'Jost',sans-serif" }}>

      <button onClick={() => { stopAll(); onBack() }} style={{ position: 'absolute', top: 16, left: 16, display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 100, padding: '7px 14px', cursor: 'pointer', color: 'rgba(255,255,255,0.45)', fontSize: 13 }}>
        ‹ Retour
      </button>

      <div style={{ fontSize: 11, letterSpacing: '.20em', textTransform: 'uppercase', color: c, marginBottom: 40, padding: '5px 16px', borderRadius: 100, border: `1px solid ${c}44`, background: `${c}14` }}>
        {zone}
      </div>

      {state === 'ready' && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 32, textAlign: 'center', padding: '0 32px' }}>
          <button onClick={startGuide} style={{ width: 100, height: 100, borderRadius: '50%', border: 'none', cursor: 'pointer', fontSize: 38, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `radial-gradient(circle at 38% 32%, rgba(255,255,255,0.18), ${c}cc, ${c}88)`, boxShadow: `0 0 0 14px ${c}18, 0 0 0 28px ${c}08, 0 16px 48px ${c}66`, transition: 'transform .2s ease' }}
            onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.06)'}
            onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
            ▶
          </button>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.50)', margin: 0, lineHeight: 1.65 }}>
            Appuyez pour démarrer<br/>le rituel guidé
          </p>
        </div>
      )}

      {state === 'playing' && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 36, textAlign: 'center', padding: '0 32px' }}>
          <div style={{ width: 160, height: 160, borderRadius: '50%', background: `radial-gradient(circle at 38% 32%, ${c}66, ${c}22, transparent)`, boxShadow: `0 0 60px ${c}44`, animation: 'breathe 4s ease-in-out infinite' }} />
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, fontStyle: 'italic', color: 'rgba(255,255,255,0.65)', margin: 0, lineHeight: 1.65 }}>
            Fermez les yeux,<br/>laissez-vous guider…
          </p>
          <button onClick={handleSkip} style={{ padding: '10px 28px', borderRadius: 100, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', cursor: 'pointer', fontSize: 13, color: 'rgba(255,255,255,0.40)' }}>
            Passer →
          </button>
        </div>
      )}
      ...
      {state === 'ended' && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, textAlign: 'center', padding: '0 40px' }}>
          <div style={{ fontSize: 64, lineHeight: 1 }}>✨</div>
          <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 30, fontWeight: 400, color: '#fff', margin: 0, lineHeight: 1.2 }}>
            Rituel accompli
          </h3>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.50)', margin: '0 0 8px', lineHeight: 1.7 }}>
            Prenez un instant pour accueillir<br/>ce qui vient de se passer en vous.
          </p>
          <button onClick={onValidate} style={{ padding: '16px 52px', borderRadius: 100, border: 'none', cursor: 'pointer', fontSize: 16, fontWeight: 700, color: '#fff', background: `linear-gradient(135deg, ${c}, ${c}88)`, boxShadow: `0 8px 28px ${c}66` }}>
            Continuer →
          </button>
        </div>
      )}
...
function DayRituel({ data, answers, dayColor, onNext, onBack, onScreenChange }) {
  ...
  const [phase,          setPhase]          = useState('view') // 'view' | 'audio' | 'guided'

  // audioId = zone en minuscules sans accents, ex: 'racines', 'tige', 'feuilles'
  const audioId = data.zone
    ? data.zone.toLowerCase().normalize('NFD').replace(/\p{Mn}/gu, '').replace(/\s+/g, '_')
    : null
  ...
  // ── Phase audio : overlay plein écran ──
  if (phase === 'audio') {
    return (
      <DayAudioPlayer
        audioId={audioId}
  ...
  // ── Phase guided : composants existants ──
  if (phase === 'guided') {
    const backToView = () => setPhase('view')
    if (data.isGuided === 'tige')       return <TigeGuidedRituel      onNext={onNext} onBack={backToView} />
    if (data.isGuided === 'feuilles')   return <FeuillesGuidedRituel  onNext={onNext} onBack={backToView} />
    if (data.isGuided === 'fleurs')     return <FleursGuidedRituel    onNext={onNext} onBack={backToView} />
    if (data.isGuided === 'souffle')    return <SouffleGuidedRituel   onNext={onNext} onBack={backToView} />
    if (data.isGuided === 'jardin')     return <JardinGuidedRituel    onNext={onNext} onBack={backToView} />
    if (data.isGuided === 'communaute') return <CommunauteGuidedRituel onNext={onNext} onBack={backToView} />
    if (data.isGuided)                  return <RacinesGuidedRituel   onNext={onNext} onBack={backToView} />
  }

  // ── Phase view : transition + écran de choix ──
  if (showTransition) {
  ...
  // Écran de présentation du rituel + bouton audio / guidé
  const buttonLabel = audioAvailable ? '🔊 Écouter le rituel guidé' : 'Commencer le rituel →'
  return (
    <div className="wof-in" style={{ padding: '8px 20px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <BackButton onClick={onBack} />

      <p style={{ fontFamily: 'Jost, sans-serif', fontSize: 11, fontWeight: 500, color: dayColor || '#c8a0b0', letterSpacing: '0.12em', textTransform: 'uppercase', margin: '8px 0 20px', textAlign: 'center' }}>
        {data.zone}
      </p>

      {data.intro && (
        <p style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 'clamp(16px, 4vw, 20px)', fontStyle: 'italic', color: '#000', textAlign: 'center', lineHeight: 1.75, margin: '0 0 32px', maxWidth: 360 }}>
          {data.intro}
        </p>
      )}

      <button
        onClick={() => audioAvailable ? setPhase('audio') : setPhase('guided')}
        style={{ padding: '16px 0', width: '100%', maxWidth: 340, borderRadius: 100, border: 'none', cursor: 'pointer', fontFamily: 'Jost, sans-serif', fontSize: 15, fontWeight: 700, color: '#fff', background: `linear-gradient(135deg, ${dayColor || '#c8a0b0'}, ${dayColor || '#a07888'})`, boxShadow: `0 6px 20px ${dayColor || '#c8a0b0'}44`, transition: 'transform .18s ease' }}
      ...
      {audioAvailable && (
        <button
          ...
          style={{ marginTop: 12, padding: '10px 24px', borderRadius: 100, background: 'transparent', border: '1px solid rgba(0,0,0,0.15)', cursor: 'pointer', fontFamily: 'Jost, sans-serif', fontSize: 13, color: 'rgba(0,0,0,0.42)' }}
        >
          Faire le rituel sans audio
        </button>
      )}
    </div>
  )
