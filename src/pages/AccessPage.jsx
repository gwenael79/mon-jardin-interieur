...
const PLANS = [
  { id: 'price_1Tds0cCIpPVJTaop0wnhmzZM', label: 'Mensuel', desc: 'Résiliable à tout moment', price: '7,99 €',  note: '/ mois', popular: false },
  { id: 'price_1Tds0tCIpPVJTaop8vbMJ7Na', label: 'Annuel',  desc: '4,92 € / mois · -38 %',  price: '59,00 €', note: '/ an',   popular: true  },
]
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes fm2-fadeIn { from{opacity:0;transform:translateY(14px)} to{opacity:1;transform:translateY(0)} }
        @keyframes fm2-formIn { from{opacity:0;transform:scale(.97)} to{opacity:1;transform:scale(1)} }
        .fm2-overlay {
          position:fixed; inset:0; z-index:1200;
          background:rgba(10,20,5,.55); backdrop-filter:blur(10px);
          display:flex; align-items:center; justify-content:center; padding:20px;
          animation:fm2-fadeIn .25s ease both;
        ...
        .fm2-modal {
          ...
          animation:fm2-formIn .35s cubic-bezier(.22,1,.36,1) both;
        }
        .fm2-close {
        ...
        .fm2-close:hover { background:rgba(0,0,0,.14); }
        .fm2-title { font-family:'Cormorant Garamond',serif; font-size:28px; font-weight:400; color:#1a1208; margin-bottom:8px; }
        .fm2-desc  { font-size:15px; color:#1a1208; margin-bottom:6px; line-height:1.65; }
        .fm2-example { font-size:13px; color:rgba(30,20,8,.40); font-style:italic; margin-bottom:18px; }
        .fm2-preview {
        ...
        .fm2-pill:hover { border-color:rgba(90,154,40,.40); color:rgba(30,20,8,.85); }
        .fm2-pill.fm2-sel { border-color:rgba(90,154,40,.55); background:rgba(90,154,40,.10); color:#2e6808; font-weight:500; }
        .fm2-cta {
        ...
        .fm2-cta:hover { filter:brightness(1.08); }
        .fm2-cta:disabled { opacity:.4; cursor:not-allowed; filter:none; }
        .fm2-footer { margin-top:14px; font-size:13px; color:rgba(30,20,8,.38); text-align:center; line-height:1.7; }
        @media(max-width:480px) {
          .fm2-modal { padding:28px 20px; }
          .fm2-grid  { max-height:180px; }
        }
      `}</style>
      <div className="fm2-overlay">
        <div className="fm2-modal">
          {onSkip && <button className="fm2-close" onClick={onSkip}>✕</button>}

          <div className="fm2-title">Votre identité florale 🌸</div>
          <div className="fm2-desc">Ici pas de nom. Chaque membre du jardin est identifié par son prénom et une fleur.</div>
          ...
          <div className="fm2-grid">
            {FLOWER_NAMES.map(name => (
              <div
                key={name}
                className={'fm2-pill' + (selectedFlower === name ? ' fm2-sel' : '')}
                onClick={() => setSelectedFlower(name)}
              >{name}</div>
            ))}
          </div>

          <button className="fm2-cta" onClick={handleConfirm} disabled={!selectedFlower || saving}>
            {saving ? 'Enregistrement…' : selectedFlower ? `Entrer dans mon jardin · ${selectedFlower} →` : 'Choisissez une fleur'}
          </button>

          <div className="fm2-footer">Modifiable dans vos paramètres.</div>
        </div>
      </div>
    </>
  )
